window.APP = {
  ...
  methods: {
    ...
    /** Called by Lua: opens settings panel with current saved values */
    ...
    /** Called by Lua on boot: silently applies saved settings (no panel open) */
    ...
    saveSettings() {
      ...
      // Save to FiveM KVP via Lua
      post('http://ef-chat/saveSettings', JSON.stringify(settings));
      // Apply visually right away
      this.applyCssSettings();
      this.closeSettings();
    },

    /** Apply CSS variables to :root so all chat elements update instantly */
    applyCssSettings() {
      const root = document.documentElement;
      const bgRgb = this.hexToRgb(this.settingsBgColor);
      if (bgRgb) {
        // Primary background with 85% opacity
        root.style.setProperty(
          '--chat-bg-primary',
          `rgba(${bgRgb.r}, ${bgRgb.g}, ${bgRgb.b}, 0.85)`
        );
        // Secondary background slightly darker (used in radial gradient second stop)
    ...
    /** Validate hex input directly in the settings fields */
    ...
    /** Recompute cpHex from current HSV state */
    cpUpdateHex() {
    ...
    /** Normalise #RGB or #RRGGBB → #RRGGBB, returns null if invalid */
    normaliseHex(hex) {
      if (!hex) return null;
      hex = hex.trim();
    ...
    /** Mouse wheel: scroll through suggestions (if visible), else do nothing */
    onWheel(e) {
    ...
    resize() {
      // placeholder – textarea resize disabled
    },
    ...
    // ══════════════════════════════════════════════
    // Suggestion helpers (shared filter)
    // ══════════════════════════════════════════════
    ...
    completeSuggestion() {
      const list = this._filterSuggestions();
